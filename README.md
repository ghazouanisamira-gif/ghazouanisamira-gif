# Samira Ghazouani

**Quality & PMO | Governance & Transformation | Operational Excellence | Risk**  
**PMP® | Lean Six Sigma Black Belt | QSE/QHSE Management**

I structure, improve and govern operational and transformation initiatives through **project management, quality, Lean Six Sigma, risk management, stakeholder coordination and data-driven decision-making**.

This GitHub profile is an **evidence hub**: selected case studies, frameworks, PMO tools, methodologies and reusable templates developed from professional and applied experience.

---

## What I Bring

- Project & PMO coordination
- Quality management and governance
- Operational excellence and performance management
- Process improvement and Lean Six Sigma
- Risk, issue and stakeholder management
- Cross-functional and multi-provider coordination
- Data analysis, KPI management and root-cause analysis
- Change management and people development
- AI-enabled operational transformation and governance

---

## Featured Evidence

| Evidence | What it demonstrates |
|---|---|
| [EMEA Quality Governance Transformation](./07-emea-quality-governance-transformation) | Multi-country governance, stakeholder coordination, quality framework, KPIs and continuous improvement |
| [AI Transformation & Governance](./09-ai-transformation-governance) | AI governance, knowledge management, phased deployment, quality, CSAT, analytics and change management |
| [Lean Six Sigma Transformation](./04-lean-six-sigma-transformation) | DMAIC, process analysis, VSM, Pareto, statistical analysis and measurable improvement |
| [Operational Structuring Case Study](./06-operational-structuring-case-study) | Operating model design, process structuring, KPIs, reporting, knowledge management and performance management |
| [Quality & Governance Framework](./02-quality-governance-framework) | Quality systems, governance routines, KPI frameworks, corrective actions and AI/human oversight |
| [Project Management Toolkit](./01-project-management-toolkit) | Reusable PMO tools, project governance templates, stakeholder and risk management |
| [People Development & Manager Enablement](./10-people-development-manager-enablement) | Capability assessment, coaching, manager enablement, learning, team development and performance development |

---

## Portfolio Map

### Project Management & PMO
- [01 — Project Management Toolkit](./01-project-management-toolkit)
- [06 — Operational Structuring Case Study](./06-operational-structuring-case-study)

### Quality, Governance & QMS
- [02 — Quality & Governance Framework](./02-quality-governance-framework)
- [07 — EMEA Quality Governance Transformation](./07-emea-quality-governance-transformation)
- [08 — ISO 9001 QMS Assessment Framework](./08-iso-9001-qms-assessment-framework)

### Operational Excellence & Process Improvement
- [03 — Operational Performance](./03-operational-performance)
- [04 — Lean Six Sigma Transformation](./04-lean-six-sigma-transformation)
- [05 — Recruitment Process Improvement](./05-recruitment-process-improvement)

### People Development & Manager Enablement
- [10 — People Development & Manager Enablement](./10-people-development-manager-enablement)

### Transformation & AI Governance
- [09 — AI Transformation & Governance](./09-ai-transformation-governance)

---

## Methodologies

**Project Management:** Project Charter, planning, RACI, stakeholder management, RAID, risk management, governance, status reporting

**Quality & Operational Excellence:** KPI management, RCA, CAPA-style action tracking, PDCA, process mapping, continuous improvement

**Lean Six Sigma:** DMAIC, SIPOC, Value Stream Mapping, Pareto analysis, 5 Whys, Fishbone, statistical analysis

**Transformation:** Change management, cross-functional coordination, multi-provider governance, knowledge management, AI adoption and human oversight

---

## Evidence Philosophy

Each case is structured around:

**Challenge → Objectives → Scope → Stakeholders → My Role → Methodology → Deliverables → Data & Analysis → Risks → Solution → Results → Lessons Learned → Transferable Skills**

Where confidential information is involved, evidence is **anonymized** and sensitive operational data is excluded.

No company, client, employee, customer, proprietary URL, credential or confidential internal information is intentionally published here.

---

## Professional Positioning

My experience comes from complex operational environments where quality, people, processes, performance and stakeholder expectations intersect.

The objective of this portfolio is to make those capabilities visible as **transferable PMO, governance, transformation and operational excellence evidence**.

---

## Connect

- [LinkedIn](https://www.linkedin.com/in/samira-ghazouani-76392472/)
- [GitHub repositories](https://github.com/ghazouanisamira-gif)

---

*Portfolio maintained as a structured professional evidence system — focused on credibility, traceability, confidentiality and continuous improvement.*
